...
class DemandasDetalhes extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isFinalizado = demanda.statusDemanda?.descricaoStatusDemanda.toUpperCase() == "FINALIZADO";
    ...
    final statusText = demanda.statusDemanda?.descricaoStatusDemanda.toUpperCase() ?? "NÃO APROVADO";
    ...
    // Estilo comum para os ListTiles
        ...
        children: [
              ...
              slivers: [
                ...
                SliverList(
                  delegate: SliverChildListDelegate([
                    ListTile(
                      shape: listTileShape,
                      title: Text('Local: ${demanda.ocorrencia?.enderecoOcorrencia ?? "N/A"}'),
                      subtitle: Text('Bairro: ${demanda.ocorrencia?.bairroOcorrencia ?? "N/A"}'),
                    ),
                    ListTile(
                      shape: listTileShape,
                      title: Text('Cidade: ${demanda.ocorrencia?.cidadeOcorrencia ?? "N/A"}'),
                      subtitle: Text('Estado: ${demanda.ocorrencia?.ufOcorrencia ?? "N/A"}'),
                    ),
                    ListTile(
                      shape: listTileShape,
                      title: Text(
                      ...
                      subtitle: Text('Horário Informado: ${demanda.ocorrencia?.horaInformadaOcorrencia ?? "N/A"}'),
                    ),
                    ListTile(
                      shape: listTileShape,
                      title: const Text('Relato do Autor'),
                      subtitle: Text(demanda.ocorrencia?.relatoAutorRegistroOcorrencia ?? "N/A"),
                    ),
                    ListTile(
                      shape: listTileShape,
                      title: const Text('Relato do Atendente'),
                      subtitle: Text(demanda.ocorrencia?.relatoAtendenteOcorrencia ?? "N/A"),
                    ),
                    ListTile(
                      shape: listTileShape,
                      title: const Text('Observações Finais'),
                      subtitle: Text(demanda.ocorrencia?.observacoesFinaisOcorrencia ?? "N/A"),
                    ),
                  ]),
                ),
              ],
          ...
          Container(
            ...
            child: ElevatedButton(
              onPressed: () {
                // Mostrar popup de confirmação antes de navegar
                showDialog(
                  context: context,
                  builder: (BuildContext context) {
                    return AlertDialog(
                      ...
                      actions: [
                        ...
                        ElevatedButton(
                          onPressed: () {
                            // Fechar o popup e navegar para a página do mapa
                            Navigator.of(context).pop();
                            Get.toNamed(
                              Routes.MAPA_DEMANDA,
                              arguments: {
